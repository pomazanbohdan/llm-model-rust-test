...
def derive_family_id(record: dict[str, object]) -> str:
    ...
    if category == "compile_repair":
        return f"{category}.{['push_first_clone', 'normalize_owned', 'join_labels_owned', 'parse_ports_collect', 'stringify_collect_owned'][idx % 5]}"
    ...
    if category == "edition2024_migration":
        return f"{category}.{['unsafe_env_set', 'unsafe_extern', 'unsafe_attribute', 'unsafe_env_remove', 'unsafe_op_in_unsafe_fn', 'static_mut_once_lock'][idx % 6]}"
    if category == "async_concurrency_fix":
        return f"{category}.{['mutex_refresh', 'channel_collect', 'retry_loop', 'read_write_lock', 'timeout_default', 'watch_shutdown'][idx % 6]}"
    ...
    if category == "cargo_workspace_fix":
        return f"{category}.{['workspace_dependency_path', 'workspace_resolver', 'workspace_members', 'workspace_inheritance', 'workspace_feature_flag'][idx % 5]}"
    if category == "rust_qa":
